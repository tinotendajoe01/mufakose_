...
function reducer(state, action) {
  switch (action.type) {
    ...
    case "CREATE_REQUEST":
      return { ...state, loadingCreate: true, user: action.payload };
    case "CREATE_SUCCESS":
...
function UserAdminServices() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const { data } = await axios.get(`/api/admin/userServices`, {
  ...
  const createHandler = async () => {
    ...
    try {
      ...
      router.push(`/admin/userService/${data.service._id}`);
    } catch (err) {
  ...
  const deleteHandler = async (serviceId) => {
    ...
    try {
      ...
      await axios.delete(`/api/admin/userServices/${serviceId}`, {
        headers: { authorization: `Bearer ${userInfo.token}` },
      });
...
export default dynamic(() => Promise.resolve(UserAdminServices), {
  ssr: false,
});
